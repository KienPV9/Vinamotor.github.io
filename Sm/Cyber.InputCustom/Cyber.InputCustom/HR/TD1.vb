...
#Region "Columns Edit"
    ...
    Dim EditMa_LydoTD, Edittien_khachsan, Edittien_dilai, Edittien_an, Edittien_khac, Edittien_daotao As New Cyber.Fill.CyberColumnGridView
    Dim EditMa_CapBac, EditMa_ChucDanh, EditTen_hs, editma_cty As New Cyber.Fill.CyberColumnGridView
#End Region
...
#Region "Load And Set And Default"
    ...
    Private Sub CreateTableMonth()
        ...
        For i As Integer = 1 To 12
            _Thang = i.ToString.Trim
            If Now.Date.Month = i Then _Default = "1" Else _Default = "0"
            DtThang.Rows.Add(_Thang.ToString, _Thang, _Thang, _Default)
        Next
        CyberFill.V_FillComBoxDefaul(cmbDen_Thang, DtThang, "Thang", IIf(Me.Lan = "V", "Ten_Thang", "Ten_Thang2"), "Default")
    ...
    Private Sub V_LoadThang()
        ...
        CyberFill.V_FillComBoxValue(Me.cmbTu_Thang, tbNam, "Ma_thang", "Ten_thang", "")
        cmbTu_Thang.Text = Month(Now.Date)
    ...
    End Sub
    Private Sub V_loaddot()
        Dim tbNam As DataTable
        Dim DsTmp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_LoadNam", M_Ma_Dvcs & "#" & M_User_Name)
        tbNam = DsTmp.Tables(2).Copy
        CyberFill.V_FillComBoxValue(Me.cmbdot, tbNam, "Ma_dot", "Ten_dot", "")
    End Sub
    ...
    Private Sub V_GetColumn()
        ...
        '
        EditMa_LydoTD.GetColumn(DetailGRV, "Ma_LyDoTD")
        EditMa_CapBac.GetColumn(DetailGRV, "Ma_CapBac")
        ...
        EditTen_hs.GetColumn(DetailGRV, "Ten_hs")
        editma_cty.GetColumn(DetailGRV, "Ma_cty")
...
#Region "V_AddHandler"
    ...
    Private Sub V_AddHandler_Master()
        ...
        AddHandler TxtMa_cty.CyberValiting, AddressOf V_Ma_cty1
        AddHandler TxtMa_cty.CyberLeave, AddressOf L_Ma_cty1
    End Sub
    Private Sub V_AddHandler_Detail()
        ...
        editma_cty.V_ActiLookUpColumn(AddressOf V_Ma_cty, AddressOf L_Ma_cty)
        '
        EditMa_CapBac.V_ActiLookUpColumn(AddressOf V_Ma_CapBac, AddressOf L_Ma_CapBac)
        EditMa_ChucDanh.V_ActiLookUpColumn(AddressOf V_Ma_Chucdanh, AddressOf L_Ma_ChucDanh)
        EditMa_LydoTD.V_ActiLookUpColumn(AddressOf V_Ma_lydotd, AddressOf L_Ma_lydotd)
...
#Region "Valid --- Ma_BP_H"
    ...
    Private Sub L_Ma_BP_H(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs)
        ...
        End If
    End Sub
    Private Sub V_Ma_cty1(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs)
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        TxtMa_cty.V_LookUp(M_LAN, M_Para, osysvar, AppConn, DsLookup, "Ma_cty", "Dmcty", "1=1", "1=1")
    End Sub
    Private Sub L_Ma_cty1(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs)
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
...
#Region "Valid - Detail - Core"
    Private Sub UpdateList()
        ...
        For iRow As Integer = 0 To Dv_DetailTmp.Count - 1
            nT_Cong = nT_Cong + Dv_DetailTmp.Item(iRow).Item("So_Luong")
        Next
...
#Region "Valid - Detail - Add On"
    ...
    End Sub
    Private Sub V_Ma_cty(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs)
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        CyberSupport.V_Lookup("Ma_cty", "Dmcty", "1=1", "1=1", M_Mode, M_Ma_CT, M_LAN, M_Para, osysvar, AppConn, DsLookup, sender)
    End Sub
    Private Sub L_Ma_cty(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If DrReturn Is Nothing Then
            Dv_DetailTmp.Item(iRow).BeginEdit()
            Dv_DetailTmp.Item(iRow).Item("Ma_cty") = ""
            Dv_DetailTmp.Item(iRow).Item("Ten_cty") = ""
            Dv_DetailTmp.Item(iRow).EndEdit()
            Exit Sub
        Else
            Dv_DetailTmp.Item(iRow).BeginEdit()
            Dv_DetailTmp.Item(iRow).Item("Ma_cty") = DrReturn("Ma_cty").ToString.Trim
            Dv_DetailTmp.Item(iRow).Item("Ten_cty") = DrReturn("Ten_cty").ToString.Trim
            Dv_DetailTmp.Item(iRow).EndEdit()
        End If
        Dv_DetailTmp.Table.AcceptChanges()
    ...
    End Sub
    'cap bac
    Private Sub V_Ma_CapBac(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs)
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        CyberSupport.V_Lookup("Ma_CapBac", "DmCapBac", "1=1", "1=1", M_Mode, M_Ma_CT, M_LAN, M_Para, osysvar, AppConn, DsLookup, sender)
    End Sub
    Private Sub L_Ma_CapBac(ByVal sender As System.Object, ByVal e As System.EventArgs)
        M_Mode = M_Mode.Trim
        If Not (M_Mode = "M" Or M_Mode = "S") Then
            Exit Sub
        End If
        ...
        If DrReturn Is Nothing Then
            Dv_DetailTmp.Item(iRow).BeginEdit()
            Dv_DetailTmp.Item(iRow).Item("Ma_CapBac") = ""
            Dv_DetailTmp.Item(iRow).Item("Ten_CapBac") = ""
            Dv_DetailTmp.Item(iRow).EndEdit()
            Exit Sub
        Else
            Dv_DetailTmp.Item(iRow).BeginEdit()
            Dv_DetailTmp.Item(iRow).Item("Ma_CapBac") = DrReturn("Ma_CapBac").ToString.Trim
            Dv_DetailTmp.Item(iRow).Item("Ten_CapBac") = DrReturn("Ten_CapBac").ToString.Trim
            Dv_DetailTmp.Item(iRow).EndEdit()
        End If
        Dv_DetailTmp.Table.AcceptChanges()
        DetailGRV.UpdateCurrentRow()
    ...
    Private Sub L_Ma_ChucDanh(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If DrReturn Is Nothing Then
            Dv_DetailTmp.Item(iRow).BeginEdit()
            Dv_DetailTmp.Item(iRow).Item("Ma_ChucDanh") = ""
            Dv_DetailTmp.Item(iRow).Item("Ten_ChucDanh") = ""
            Dv_DetailTmp.Item(iRow).EndEdit()
            Exit Sub
        Else
            Dv_DetailTmp.Item(iRow).BeginEdit()
            Dv_DetailTmp.Item(iRow).Item("Ma_ChucDanh") = DrReturn("Ma_ChucDanh").ToString.Trim
            Dv_DetailTmp.Item(iRow).Item("Ten_ChucDanh") = DrReturn("Ten_ChucDanh").ToString.Trim
            Dv_DetailTmp.Item(iRow).EndEdit()
        End If
        Dv_DetailTmp.Table.AcceptChanges()
        DetailGRV.UpdateCurrentRow()
    End Sub
    'ly do td
    Private Sub V_Ma_LydoTD(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs)
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        CyberSupport.V_Lookup("Ma_LydoTD", "DmLydoTD", "1=1", "1=1", M_Mode, M_Ma_CT, M_LAN, M_Para, osysvar, AppConn, DsLookup, sender)
    End Sub
    Private Sub L_Ma_LydoTD(ByVal sender As System.Object, ByVal e As System.EventArgs)
        M_Mode = M_Mode.Trim
        If Not (M_Mode = "M" Or M_Mode = "S") Then
            Exit Sub
        End If
        ...
        If DrReturn Is Nothing Then
            Dv_DetailTmp.Item(iRow).BeginEdit()
            Dv_DetailTmp.Item(iRow).Item("Ma_LydoTD") = ""
            Dv_DetailTmp.Item(iRow).Item("Ten_LydoTD") = ""
            Dv_DetailTmp.Item(iRow).EndEdit()
            Exit Sub
        Else
            Dv_DetailTmp.Item(iRow).BeginEdit()
            Dv_DetailTmp.Item(iRow).Item("Ma_LydoTD") = DrReturn("Ma_LydoTD").ToString.Trim
            Dv_DetailTmp.Item(iRow).Item("Ten_LydoTD") = DrReturn("Ten_LydoTD").ToString.Trim
            Dv_DetailTmp.Item(iRow).EndEdit()
        End If
        Dv_DetailTmp.Table.AcceptChanges()
        DetailGRV.UpdateCurrentRow()
...
#Region "Button"
    ...
    Private Function ChkVsave() As Boolean
        ...
        For iRow = nCount - 1 To 0 Step -1
            If Dt_DetailTmp.Rows(iRow).Item("Ma_CapBac").ToString.Trim = "" Then
