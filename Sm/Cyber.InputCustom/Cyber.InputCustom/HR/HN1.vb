...
#Region "Load And Set And Default"
    ...
    Private Sub V_SetFocus(ByVal _Loai As String)
        ...
        Select Case _Loai
            Case "M"
                If DrDmct.Item("M_Ngay_CT").ToString.Trim <> "1" Then TxtNgay_Ct.Focus() Else TxtMa_HS_i.Focus()
            Case "S"
                If DrDmct.Item("M_Ngay_CT").ToString.Trim <> "1" Then TxtNgay_Ct.Focus() Else TxtMa_HS_i.Focus()
            Case "L"
                CmdPrint.Select()
...
#Region "V_AddHandler"
    ...
    Private Sub V_AddHandler_Master()
        ...
        '-------------------------------------------------------------------------------------
        '--Ma_BP_H
        AddHandler TxtMa_Bp_i.CyberValiting, AddressOf V_Ma_BP_H
        AddHandler TxtMa_Bp_i.CyberLeave, AddressOf L_Ma_BP_H
        '--Ma_HS_H
        AddHandler TxtMa_HS_i.CyberValiting, AddressOf V_Ma_HS_H
        AddHandler TxtMa_HS_i.CyberLeave, AddressOf L_Ma_HS_H

        '--Ma_Chucvu_Old

        'AddHandler TxtGio_ra.Leave, AddressOf V_Gio_Vao
        '--Ma_dvcs_di
...
#Region "Valid --- Ma_BP_H"
    Private Sub V_Ma_BP_H(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs)
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        TxtMa_Bp_i.V_LookUp(M_LAN, M_Para, osysvar, AppConn, DsLookup, "Ma_BP", "DmBP", "1=1", "1=1")
    End Sub
    Private Sub L_Ma_BP_H(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs)
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        If TxtMa_Bp_i.Text = "" Then
            TxtTen_Bp.Text = ""

            Exit Sub
        End If
        DrReturn = TxtMa_Bp_i.GetRowsSelectData(True)
        If Not DrReturn Is Nothing Then
            TxtMa_Bp_i.Text = DrReturn.Item("Ma_BP")
...
#Region "Valid --- Ma_HS_H"
    Private Sub V_Ma_HS_H(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs)
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        TxtMa_HS_i.V_LookUp(M_LAN, M_Para, osysvar, AppConn, DsLookup, "Ma_HS", "DmHS", "1=1", "1=1")
    End Sub
...
#Region "Button"
    ...
    Private Sub V_GetMaBP()
        ...
        If dsTMp.Tables(0).Rows.Count < 1 Then
            ...
            Exit Sub
        End If
        If dsTMp.Tables(0).Columns.Contains("Ma_Bp_H") Then If TxtMa_Bp_i.Text.Trim = "" Then TxtMa_Bp_i.Text = dsTMp.Tables(0).Rows(0).Item("Ma_Bp_H")
        ...
        If dsTMp.Tables(0).Columns.Contains("Ma_Hs_H") Then If TxtMa_HS_i.Text.Trim = "" Then TxtMa_HS_i.Text = dsTMp.Tables(0).Rows(0).Item("Ma_HS_H")
