...
#Region "Columns Edit"
    ...
    Dim EditTien_hoa_hong_TNDS, EditTien_hoa_hong_VCX, EditTien_MG, EditTien_tru As New Cyber.Fill.CyberColumnGridView
...
#Region "Load And Set And Default"
    ...
    Private Sub V_loadNV(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If (M_Mode <> "M" And M_Mode <> "S") Then
            Exit Sub
        End If
        Dim DsTmp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_Get_TBH", CbbMa_GD.SelectedValue & "#" & cmbNam.SelectedValue & "#" & cmdthang.SelectedValue & "#" & M_Ma_Dvcs & "#" & M_User_Name)
...
#Region "Valid - Detail - Core"
    Private Sub UpdateList()
        Dim _nThuong As Double = 0
        Dim _nphat As Double = 0
        For iRow As Integer = 0 To Dv_DetailTmp.Count - 1
            Dv_DetailTmp.Item(iRow).BeginEdit()
            Dv_DetailTmp.Item(iRow).Item("Tien_thuong") = Dv_DetailTmp.Item(iRow).Item("Tien_hoa_hong_TNDS") + Dv_DetailTmp.Item(iRow).Item("Tien_hoa_hong_VCX") + Dv_DetailTmp.Item(iRow).Item("Tien_tru") + Dv_DetailTmp.Item(iRow).Item("Tien_MG")
            If Dv_DetailTmp.Item(iRow).Item("Tien_thuong") >= 0 Then
                Dv_DetailTmp.Item(iRow).Item("tien_dc_thuong") = Dv_DetailTmp.Item(iRow).Item("Tien_thuong")
                _nThuong = _nThuong + Dv_DetailTmp.Item(iRow).Item("tien_dc_thuong")
            Else
                Dv_DetailTmp.Item(iRow).Item("tien_bi_phat") = Dv_DetailTmp.Item(iRow).Item("Tien_thuong")
                _nphat = _nphat + Dv_DetailTmp.Item(iRow).Item("tien_bi_phat")
            End If
        ...
        TxtT_THUONG.Double = _nThuong
        TxtT_phat.Double = _nphat
...
#Region "Sms - Import"
    ...
    Private Sub SetTxt()
        ...
        TxtT_THUONG.ReadOnly = True
        TxtT_phat.ReadOnly = True
    End Sub
#End Region
