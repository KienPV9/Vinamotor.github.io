...
#Region "V_AddHandler"
    ...
    Private Sub V_AddHandler_Master()
        ...
        AddHandler Txtso_yc.Leave, AddressOf V_So_YC
    End Sub
...
#Region "Valid --- Ma_HD_H/Ma_Bp_H/Ma_HS_H"
    ...
    Private Sub L_Ma_TVBH(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs)
        ...
        If Not DrReturn Is Nothing Then
            ...
            If TxtMa_TVBH.Text <> "" Then TxtTen_TVBH.Text = CyberSmodb.SQLGetvalue(AppConn, "Ten_hs", "Dmhs", "Ma_hs = N'" + TxtMa_TVBH.Text.Trim + "'", CyberSmlib)
        Else
            TxtMa_TVBH.Text = ""
            TxtTen_TVBH.Text = ""
...
#Region "Tich diem"
    ...
    End Sub
    Private Sub V_So_YC(ByVal sender As System.Object, ByVal e As System.EventArgs)
        M_Mode = M_Mode.Trim
        If Not (M_Mode = "M") Then Exit Sub
        GetDetailYCPK()
    End Sub
    Private Sub GetDetailYCPK()
        If Txtso_yc.Text = "" Then
            Exit Sub
        End If
        ...
        DsTmpDetal = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_GetYCLDPK", Txtso_yc.Text.Trim + "#" + dt.ToString("yyyyMMdd") + "#" +
            M_Mode + "#" + M_Ma_Dvcs.Trim + "#" + M_User_Name)
        ...
        If DsTmpDetal.Tables(2).Rows.Count < 1 And DsTmpDetal.Tables(3).Rows.Count < 1 Then
            DsTmpDetal.Dispose()
            Exit Sub
        End If
        TxtMa_Xe.Text = IIf(TxtMa_Xe.Text = "", DsTmpDetal.Tables(1).Rows(0).Item("Ma_xe").ToString.Trim, TxtMa_Xe.Text)
        TxtMa_TVBH.Text = IIf(TxtMa_TVBH.Text = "", DsTmpDetal.Tables(1).Rows(0).Item("Ma_TVBH").ToString.Trim, TxtMa_TVBH.Text)
        TxtMa_LHKH.Text = IIf(TxtMa_LHKH.Text = "", DsTmpDetal.Tables(1).Rows(0).Item("Ma_LHKH").ToString.Trim, TxtMa_LHKH.Text)
        TxtTen_TVBH.Text = IIf(TxtTen_TVBH.Text = "", DsTmpDetal.Tables(1).Rows(0).Item("Ten_TVBH").ToString.Trim, TxtTen_TVBH.Text)
        TxtSo_khung.Text = IIf(TxtSo_khung.Text = "", DsTmpDetal.Tables(1).Rows(0).Item("So_Khung").ToString.Trim, TxtSo_khung.Text)
        TxtSo_May.Text = IIf(TxtSo_May.Text = "", DsTmpDetal.Tables(1).Rows(0).Item("So_May").ToString.Trim, TxtSo_May.Text)
        TxtTen_kh.Text = IIf(TxtTen_kh.Text = "", DsTmpDetal.Tables(1).Rows(0).Item("Ten_Kh").ToString.Trim, TxtTen_kh.Text)
        TxtTen_khVAT.Text = IIf(TxtTen_khVAT.Text = "", DsTmpDetal.Tables(1).Rows(0).Item("Ten_KhVAT").ToString.Trim, TxtTen_khVAT.Text)
        TxtDia_Chi.Text = IIf(TxtDia_Chi.Text = "", DsTmpDetal.Tables(1).Rows(0).Item("Dia_chi").ToString.Trim, TxtDia_Chi.Text)
        TxtDia_ChiVAT.Text = IIf(TxtDia_ChiVAT.Text = "", DsTmpDetal.Tables(1).Rows(0).Item("Dia_ChiVAT").ToString.Trim, TxtDia_ChiVAT.Text)
        TxtDt_LX.Text = IIf(TxtDt_LX.Text = "", DsTmpDetal.Tables(1).Rows(0).Item("Dien_thoai").ToString.Trim, TxtDt_LX.Text)
        TxtDien_Thoai.Text = IIf(TxtDien_Thoai.Text = "", DsTmpDetal.Tables(1).Rows(0).Item("Dien_thoai").ToString.Trim, TxtDien_Thoai.Text)
        TxtMa_HD_H.Text = IIf(TxtMa_HD_H.Text = "", DsTmpDetal.Tables(1).Rows(0).Item("ma_hd").ToString.Trim, TxtMa_HD_H.Text)

        Dt_DetailTmp.Clear()
        CyberSmodb.DeleteDatatable(Dt_DetailTmp, "Stt_Rec = '" + M_Stt_Rec + "'")
        CyberSmodb.SQLTbToTb(DsTmpDetal.Tables(2), Dt_DetailTmp)
        CyberSmodb.SQLTbToTb(DsTmpDetal.Tables(3), Dt_DetailCvTmp)
        DsTmpDetal.Dispose()
