...
#Region "Columns Edit"
    ...
    Dim EditMa_Db_I, EditMa_Xe_I, EditMa_Kx_I, EditMa_Tuyen_I, EditMa_Ts, EditMa_Cc, EditSo_Khung As New Cyber.Fill.CyberColumnGridView
...
#Region "V_AddHandler"
    ...
    Private Sub VisibleColumnMa_GD()
        If CmbMa_GD.SelectedValue = "5" Then
            EditTk_VT.Visible = False
            EditMa_Nx_I.Visible = False
        Else
            EditTk_VT.Visible = True
            EditMa_Nx_I.Visible = True
        End If
    End Sub
...
#Region "So_khung"
    Private Sub L_So_khung(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        iRow = DetailGRV.GetFocusedDataSourceRowIndex
        If iRow < 0 Then Exit Sub
        Dim _So_Khung As String = Dv_DetailTmp.Item(iRow)("So_khung")
        Dim _DtTab As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "Cp_GetIn4fromSo_Khung", TxtMa_Kho.Text & "#" & _So_Khung.Trim & "#" & M_Ma_Dvcs.ToString().Trim() & "#" & M_User_Name.ToString().Trim())
        Dim _tb As DataTable = _DtTab.Tables(0)
        If _tb.Rows.Count <= 0 Then
            If _tb.Columns.Contains("So_may") Then Dv_DetailTmp.Item(iRow).Item("So_may") = ""
            If _tb.Columns.Contains("Ma_Kx") Then Dv_DetailTmp.Item(iRow).Item("Ma_Kx") = ""
            If _tb.Columns.Contains("TEN_KX") Then Dv_DetailTmp.Item(iRow).Item("TEN_KX") = ""
            If _tb.Columns.Contains("Ma_Mau") Then Dv_DetailTmp.Item(iRow).Item("Ma_Mau") = ""
            If _tb.Columns.Contains("TEN_MAU") Then Dv_DetailTmp.Item(iRow).Item("TEN_MAU") = ""
            If _tb.Columns.Contains("TON13") Then Dv_DetailTmp.Item(iRow).Item("TON13") = ""
            Exit Sub
        End If
        Dv_DetailTmp.Item(iRow).BeginEdit()
        If _tb.Columns.Contains("So_may") Then Dv_DetailTmp.Item(iRow).Item("So_may") = _tb.Rows(0).Item("So_may")
        If _tb.Columns.Contains("Ma_Kx") Then Dv_DetailTmp.Item(iRow).Item("Ma_Kx") = _tb.Rows(0).Item("Ma_Kx")
        If _tb.Columns.Contains("TEN_KX") Then Dv_DetailTmp.Item(iRow).Item("TEN_KX") = _tb.Rows(0).Item("TEN_KX")
        If _tb.Columns.Contains("Ma_Mau") Then Dv_DetailTmp.Item(iRow).Item("Ma_Mau") = _tb.Rows(0).Item("Ma_Mau")
        If _tb.Columns.Contains("TEN_MAU") Then Dv_DetailTmp.Item(iRow).Item("TEN_MAU") = _tb.Rows(0).Item("TEN_MAU")
        If _tb.Columns.Contains("TON13") Then Dv_DetailTmp.Item(iRow).Item("TON13") = _tb.Rows(0).Item("TON13")
    End Sub
#End Region
