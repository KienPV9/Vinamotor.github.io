...
#Region "Columns Edit"
    ...
    Dim EditPS_NT, EditPs, EditSo_Luong, EditSl_Td_i, EditGia_NT, EditGia, EditTien_NT, EditTien, EditHe_So1, EditGia_NT2, EditGia2, EditTien_NT2, EditTien2, EditGia_NT3, EditGia3, EditTien_NT3, EditTien3, EditDien_Giaii, EditSo_So, EditSo_Po, EditSo_Ro, EditSo_VT As New Cyber.Fill.CyberColumnGridView 'DevExpress.XtraEditors.Repository.RepositoryItemTextEdit
    Dim EditTen_Vt, EditDvt, EditTen_kho_i, EditTen_Tk_VT, EditTen_Nx_I As New Cyber.Fill.CyberColumnGridView
    Dim EditMa_Nh_Kx2, EditTen_Nh_Kx2, EditMa_Mau, EditTen_Mau As New Cyber.Fill.CyberColumnGridView
#End Region
...
#End Region
    ...
    Private Sub DKX_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        CreateTableMonth()
        V_Load()
        V_GetColumn()
        V_AddHandler()
...
#Region "V_AddHandler"
    ...
    Private Sub V_AddHandler_Detail()
        ...
        If Not EditTien_NT.Column Is Nothing Then AddHandler EditTien_NT.EditColumn.Leave, AddressOf V_CSDKDK
...
#Region "Sms - Import"
    ...
    End Sub
    Private Sub V_CSDKDK(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        Tinh_CSDKDK(iRow)
    End Sub
    Private Sub Tinh_CSDKDK(ByVal iRow As Integer)
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        If iRow < 0 Then Exit Sub
        ...
        Dim _Ngay_Ct As Date = TxtNgay_Ct.Value
        ''-----------------------------------------------------------------------------------------------------------
        Dim dsCSDKDK As New DataSet
        dsCSDKDK = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_GetCSDKDK", _Ngay_Ct.ToString("yyyyMMdd") & "#" & M_Ma_CT.Trim & "#" & M_Stt_Rec & "#" & M_Mode & "#" & _
                                                    _Ma_GD & "#" & _Ma_Post & "#" & _Tien_NT.ToString.Trim.Replace(" ", "").Replace(",", ".") & "#" & TxtMa_Dvcs.Text.Trim + "#" + M_User_Name.Trim)

        dsCSDKDK.Dispose()
        ''-----------------------------------------------------------------------------------------------------------
        Dv_DetailTmp.Item(iRow).BeginEdit()
        Dv_DetailTmp.Item(iRow).Item("Tien_CT") = dsCSDKDK.Tables(0).Rows(0).Item("Tien_CT")
        Dv_DetailTmp.Item(iRow).Item("tien_GDDL") = dsCSDKDK.Tables(0).Rows(0).Item("tien_GDDL")
        Dv_DetailTmp.Item(iRow).Item("tien_GDBH") = dsCSDKDK.Tables(0).Rows(0).Item("tien_GDBH")
        Dv_DetailTmp.Item(iRow).Item("tien_TVBH") = dsCSDKDK.Tables(0).Rows(0).Item("tien_TVBH")
        Dv_DetailTmp.Item(iRow).Item("tien_NVDK") = dsCSDKDK.Tables(0).Rows(0).Item("tien_NVDK")
        Dv_DetailTmp.Item(iRow).Item("tien_Admin") = dsCSDKDK.Tables(0).Rows(0).Item("tien_Admin")
        Dv_DetailTmp.Item(iRow).Item("tien_PKT") = dsCSDKDK.Tables(0).Rows(0).Item("tien_PKT")
