...
#End Region
#Region "Columns Edit"
    Dim EditMa_KH, EditMa_HS, EditMa_HD, EditMa_TD3, EditMa_NH, EditNgay_GN, EditNgay_Chi, EditMa_HS_TP,
        EditTien_TVBH, EditTien_TNBH, EditTien_TPBH, EditTien_HoTro, EditDien_Giaii, EditNgay_TBCV, EditNgay_Gui_Bank, EditTien_Vay As New Cyber.Fill.CyberColumnGridView
#End Region
#Region "Khai bao bien Private"
    Dim M_Ct, M_Ph As String
    Dim DrReturn As DataRow
    Dim M_Count As Integer = 1
    Dim DtPost, DtMaGD, DtXepLoai As DataTable
#End Region
...
#Region "Load And Set And Default"
    ...
    Private Sub V_GetColumn()
        ...
        EditMa_NH.GetColumn(DetailGRV, "Ma_NH")
        EditNgay_GN.GetColumn(DetailGRV, "Ngay_GN")
        EditNgay_Chi.GetColumn(DetailGRV, "Ngay_Chi")
        EditMa_HS.GetColumn(DetailGRV, "Ma_HS")
        EditMa_HS_TP.GetColumn(DetailGRV, "Ma_HS_TP")
        EditTien_TVBH.GetColumn(DetailGRV, "Tien_TVBH")
        EditTien_TNBH.GetColumn(DetailGRV, "Tien_TNBH")
        EditTien_TPBH.GetColumn(DetailGRV, "Tien_TPBH")
        EditTien_HoTro.GetColumn(DetailGRV, "Tien_HoTro")
        EditNgay_TBCV.GetColumn(DetailGRV, "Ngay_TBCV")
        EditNgay_Gui_Bank.GetColumn(DetailGRV, "Ngay_Gui_Bank")
        EditTien_Vay.GetColumn(DetailGRV, "Tien_Vay")
...
#Region "V_AddHandler"
    ...
    Private Sub V_AddHandler_Detail()
        '---Add On
        If Not EditTien_HoTro.Column Is Nothing Then AddHandler EditTien_HoTro.EditColumn.Leave, AddressOf L_Tinh_Toan
        If Not EditTien_TPBH.Column Is Nothing Then AddHandler EditTien_TPBH.EditColumn.Leave, AddressOf L_Tinh_Toan
        If Not EditTien_TVBH.Column Is Nothing Then AddHandler EditTien_TVBH.EditColumn.Leave, AddressOf L_Tinh_Toan
        If Not EditTien_TNBH.Column Is Nothing Then AddHandler EditTien_TNBH.EditColumn.Leave, AddressOf L_Tinh_Toan
        If Not EditTien_Vay.Column Is Nothing Then AddHandler EditTien_TNBH.EditColumn.Leave, AddressOf L_Tinh_Toan

        EditMa_HD.V_ActiLookUpColumn(AddressOf V_Ma_HD, AddressOf L_Ma_HD)
        EditMa_NH.V_ActiLookUpColumn(AddressOf V_Ma_NH, AddressOf L_Ma_NH)
        EditMa_HS_TP.V_ActiLookUpColumn(AddressOf V_Ma_HS_TP, AddressOf L_Ma_HS_TP)
...
#Region "Valid - Detail - Core"
    Private Sub UpdateList()
        Dim N_Tong_Tien As Decimal = 0
        Dim n_T_TVSP As Decimal = 0
        Dim n_T_TPKD As Decimal = 0
        Dim n_T_PPKD As Decimal = 0
        Dim n_T_HTTD As Decimal = 0
        Dim n_Tien_Vay As Decimal = 0
        For iRow As Integer = 0 To Dv_DetailTmp.Count - 1
            n_T_TVSP = n_T_TVSP + Dv_DetailTmp.Item(iRow).Item("Tien_TVBH")
            n_T_TPKD = n_T_TPKD + Dv_DetailTmp.Item(iRow).Item("Tien_TNBH")
            n_T_PPKD = n_T_PPKD + Dv_DetailTmp.Item(iRow).Item("Tien_TPBH")
            n_T_HTTD = n_T_HTTD + Dv_DetailTmp.Item(iRow).Item("Tien_HoTro")
            n_Tien_Vay = n_Tien_Vay + Dv_DetailTmp.Item(iRow).Item("Tien_Vay")
            N_Tong_Tien = n_T_TVSP + n_T_TPKD + n_T_PPKD + n_T_HTTD
        Next

        TxtTong_tien.Double = N_Tong_Tien
    ...
    Private Sub L_Tinh_Toan(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        UpdateList()
    End Sub
    Private Sub V_TTHopDong(ByVal iRow As Integer)
        If Not (M_Mode.Trim = "M" Or M_Mode.Trim = "S") Then Exit Sub
        If iRow < 0 Then Exit Sub
        Dim _strF As String = ""
        Dim _strV As String = ""
        Dim _Ngay_Ct As Date = TxtNgay_Ct.Value
        Dim _Ma_HD As String = Dv_DetailTmp.Item(iRow).Item("Ma_HD").ToString.Trim()
        CyberSmodb.GetValueData(Dv_DetailTmp.Item(iRow), _strF, _strV, "Ten_KH,Ten_HS")
        Dim Dstmp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_GetTTHopDong", _Ma_HD.Trim + "#" + _Ngay_Ct.ToString("yyyyMMdd") + "#" + M_Mode + "#" + _strF + "#" + _strV + "#" + TxtMa_Dvcs.Text + "#" + M_User_Name)
...
#Region "Valid - Detail - Add On"
    Private Sub V_Ma_HD(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs)
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        CyberSupport.V_Lookup("Ma_HD", "DmHd", "1=1", "Tra_Gop='1'", M_Mode, M_Ma_CT, M_LAN, M_Para, osysvar, AppConn, DsLookup, sender)
    End Sub
    Private Sub L_Ma_HD(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If DrReturn Is Nothing Then
            If Dv_DetailTmp.Item(iRow).Item("Ma_HD").ToString.Trim = "" Then
        ...
        Dv_DetailTmp.Item(iRow).Item("Ma_HD") = DrReturn("Ma_HD").ToString.Trim
    ...
    Private Sub V_Ma_TD3(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs)
        ...
        iRow = DetailGRV.GetFocusedDataSourceRowIndex
        If iRow < 0 Then Exit Sub
        Dim _ma_hd As String = Dv_DetailTmp.Item(iRow).Item("Ma_HD")
        CyberSupport.V_Lookup("Ma_TD3", "DMTD3", "1=1", "Ma_Hd='" + _ma_hd + "'", M_Mode, M_Ma_CT, M_LAN, M_Para, osysvar, AppConn, DsLookup, sender)
    End Sub
    Private Sub L_Ma_TD3(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If iRow < 0 Then Exit Sub
        Dv_DetailTmp.Item(iRow).BeginEdit()
        Dv_DetailTmp.Item(iRow).Item("Ma_TD3") = DrReturn("Ma_TD3")

        Dv_DetailTmp.Item(iRow).EndEdit()
        Dv_DetailTmp.Table.AcceptChanges()
    End Sub
    Private Sub V_Ma_NH(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs)
        CyberSupport.V_Lookup("Ma_NH", "DmnhTraGop", "1=1", "1=1", M_Mode, M_Ma_CT, M_LAN, M_Para, osysvar, AppConn, DsLookup, sender)
    End Sub
    ...
    Private Sub L_Ma_NH(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If iRow < 0 Then Exit Sub
        Dv_DetailTmp.Item(iRow).BeginEdit()
        Dv_DetailTmp.Item(iRow).Item("Ma_NH") = DrReturn("Ma_NH")

        Dv_DetailTmp.Item(iRow).EndEdit()
        Dv_DetailTmp.Table.AcceptChanges()
    End Sub
    Private Sub V_Ma_HS_TP(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs)
        CyberSupport.V_Lookup("Ma_HS", "DMHS", "Is_KD_TP='1'", "1=1", M_Mode, M_Ma_CT, M_LAN, M_Para, osysvar, AppConn, DsLookup, sender)
    End Sub

    Private Sub L_Ma_HS_TP(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        DrReturn = Nothing
        DrReturn = CyberSupport.L_Lookup(M_Mode, M_Ma_CT, sender, True)
        Dim iRow As Integer = -1
        ...
        If iRow < 0 Then Exit Sub
        Dv_DetailTmp.Item(iRow).BeginEdit()
        Dv_DetailTmp.Item(iRow).Item("Ma_HS_TP") = DrReturn("Ma_HS")
        Dv_DetailTmp.Item(iRow).Item("Ten_Hs_TP") = DrReturn("Ten_Hs")
    ...
    Private Sub L_Ma_HS(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If iRow < 0 Then Exit Sub
        Dv_DetailTmp.Item(iRow).BeginEdit()
        Dv_DetailTmp.Item(iRow).Item("Ma_HS") = DrReturn("Ma_HS")
        Dv_DetailTmp.Item(iRow).Item("Ten_Hs") = DrReturn("Ten_Hs")
    ...
    Private Sub L_Ma_KH(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If iRow < 0 Then Exit Sub
        Dv_DetailTmp.Item(iRow).BeginEdit()
        Dv_DetailTmp.Item(iRow).Item("Ma_KH") = DrReturn("Ma_KH")
        Dv_DetailTmp.Item(iRow).Item("Ten_KH") = DrReturn("Ten_KH")
...
#Region "Button"
    ...
    Private Function ChkVsave() As Boolean
        ...
        For iRow = 0 To Dt_DetailTmp.Rows.Count - 1
            ...
            Dt_DetailTmp.Rows(iRow).Item("ma_post") = CbbMa_Post.SelectedValue.ToString.Trim
    ...
    Private Sub V_New(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        Dim Ma_GD As String = "", m_ma_xly As String = ""
        Ma_GD = CbbMa_GD.SelectedValue
        'Dim DsTmp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_LoadTieu_chi", M_Stt_Rec & "#" & M_Ma_CT & "#" & M_Mode & "#" & M_Ma_Dvcs & "#" & M_User_Name)
        'For i As Integer = 0 To DsTmp.Tables.Count - 1
        '    CyberSmodb.SetNotNullTable(DsTmp.Tables(i))
        'Next
        'Dt_DetailTmp.Clear()
    ...
    Private Sub V_New()
        If Dt_DetailTmp.Rows.Count < 1 Then
            ...
            DrCurren.Item("Ma_DVCS") = M_Ma_Dvcs.Trim
