...
#Region "Valid - Detail - Add On"
    ...
    Private Sub L_Ma_TD3_i(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        drReturn = CyberSupport.L_Lookup(M_Mode, M_Ma_CT, sender, True)
        If drReturn Is Nothing Then
            Dt_DetailTmp.Rows(iRows)("ma_td3_i") = ""
            Dt_DetailTmp.AcceptChanges()
            Exit Sub
        End If
        Dt_DetailTmp.Rows(iRows)("ma_td3_i") = drReturn("ma_td3").ToString.Trim
        Dt_DetailTmp.AcceptChanges()
        DetailGRV.UpdateCurrentRow()
        V_GetIn4_xe(iRows)
    End Sub
    Private Sub V_GetIn4_xe(ByVal iRow As Integer)
        M_Mode = M_Mode.Trim
        If iRow < 0 Then Exit Sub
        If iRow < 0 Then Exit Sub
        Dim _Ma_HD As String = If(Dt_DetailTmp.Columns.Contains("MA_HD_I"), Dv_DetailTmp.Item(iRow).Item("MA_HD_I").ToString.Trim, "")
        Dim _Ma_td3 As String = If(Dt_DetailTmp.Columns.Contains("MA_td3_I"), Dv_DetailTmp.Item(iRow).Item("MA_td3_I").ToString.Trim, "")
        Dim DsMa_td3 As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_GetIn4Xe_ma_td3", _Ma_HD & "#" & _Ma_td3 & "#" & M_Ma_Dvcs & "#" & M_User_Name)
        If DsMa_td3.Tables(0).Rows.Count = 0 Then Exit Sub
        Dt_DetailTmp.Rows(iRow).BeginEdit()
        Dt_DetailTmp.Rows(iRow).Item("So_Khung") = DsMa_td3.Tables(0).Rows(0).Item("So_Khung")
        Dt_DetailTmp.Rows(iRow).Item("So_May") = DsMa_td3.Tables(0).Rows(0).Item("So_May")
        Dt_DetailTmp.Rows(iRow).Item("so_may2") = DsMa_td3.Tables(0).Rows(0).Item("so_may2")
        Dt_DetailTmp.Rows(iRow).Item("ma_kx") = DsMa_td3.Tables(0).Rows(0).Item("ma_kx")
        Dt_DetailTmp.Rows(iRow).Item("Ten_kx") = DsMa_td3.Tables(0).Rows(0).Item("Ten_kx")
        Dt_DetailTmp.Rows(iRow).Item("ma_mau") = DsMa_td3.Tables(0).Rows(0).Item("ma_mau")
        Dt_DetailTmp.Rows(iRow).Item("ten_mau") = DsMa_td3.Tables(0).Rows(0).Item("ten_mau")
        Dt_DetailTmp.Rows(iRow).EndEdit()
    End Sub
