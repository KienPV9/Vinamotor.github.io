...
#End Region
#Region "Columns Edit"
    Dim EditMa_BP_I, EditMa_HS_I, editma_chucvu, editma_chucdanh, editis_nghi, editTham_nien, editSL_VP, editDiem_tru, editDiem_con_lai, editSL_VP_QL, editDiem_chuan, editDiem_DG As New Cyber.Fill.CyberColumnGridView
    Dim Editma_TP, EditMA_PTLH, Editma_quan, Editma_xa, Editma_kx, Editma_mau, Editten_hs_i, Editten_xa, EditTen_quan, EditTen_TP, Edituser_name As New Cyber.Fill.CyberColumnGridView
#End Region
...
#Region "Load And Set And Default"
    ...
    'Private Sub v_mucGD()
    '    Dim Ma_GD As String = "", m_ma_xly As String = ""
    '    Ma_GD = CbbMa_GD.SelectedValue
    '    Dim DsTmp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_LoadMuc_GD1", M_Stt_Rec & "#" & M_Ma_CT & "#" & Ma_GD & "#" & M_Mode & "#" & M_Ma_Dvcs & "#" & M_User_Name)
    ...
    Private Sub V_GetColumn()
        ...
        editDiem_chuan.GetColumn(DetailGRV, "Diem_chuan")
        editTham_nien.GetColumn(DetailGRV, "Tham_nien")
        Editma_TP.GetColumn(DetailGRV, "Ma_TP")
        EditMA_PTLH.GetColumn(DetailGRV, "MA_PTLH")
        Editma_quan.GetColumn(DetailGRV, "Ma_Quan")
        Editma_xa.GetColumn(DetailGRV, "Ma_xa")
        Editma_kx.GetColumn(DetailGRV, "Ma_kx")
        Editma_mau.GetColumn(DetailGRV, "Ma_mau")
        EditMa_HS_I.GetColumn(DetailGRV, "Ma_hs_i")
        Editten_hs_i.GetColumn(DetailGRV, "Ten_hs_i")
        EditTen_TP.GetColumn(DetailGRV, "Ten_tp")
        EditTen_quan.GetColumn(DetailGRV, "Ten_quan")
        Editten_xa.GetColumn(DetailGRV, "Ten_xa")
...
#Region "V_AddHandler"
    ...
    Private Sub V_AddHandler_Master()
        ...
        ' AddHandler CbbMa_GD.SelectedValueChanged, AddressOf v_mucGD
        AddHandler TxtThoi_Gian_SK.Leave, AddressOf V_Gio_II
        '----------------------------------------------------------------------------------------------------------
    End Sub
    Private Sub V_AddHandler_Detail()
        ...
        Editma_TP.V_ActiLookUpColumn(AddressOf V_Ma_tp_i, AddressOf L_Ma_tp_i)
        Editma_quan.V_ActiLookUpColumn(AddressOf V_Ma_quan_i, AddressOf L_Ma_quan_i)
        Editma_xa.V_ActiLookUpColumn(AddressOf V_Ma_xa_i, AddressOf L_Ma_xa_i)
        ...
        EditMA_PTLH.V_ActiLookUpColumn(AddressOf V_MA_PTLH, AddressOf L_MA_PTLH)

        Editma_mau.V_ActiLookUpColumn(AddressOf V_Ma_mau, AddressOf L_Ma_mau)
        EditMa_HS_I.V_ActiLookUpColumn(AddressOf V_Ma_name, AddressOf L_Ma_name)
...
#Region "Valid --- Ma_BP_H"
    ...
    Private Sub V_Ma_sk(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs)
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        Txtma_sk.V_LookUp(M_LAN, M_Para, osysvar, AppConn, DsLookup, "Ma_sk", "Dmsk", "1=1", "1=1")
    End Sub
    Private Sub L_Ma_sk(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs)
        ...
        If Txtma_sk.Text = "" Then
            ...
            Exit Sub
        End If
        DrReturn = Txtma_sk.GetRowsSelectData(True)
        If Not DrReturn Is Nothing Then
...
#End Region
#Region "Giờ"
    Private Sub V_Gio_II(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        TxtThoi_Gian_SK.Text = CyberVoucher.V_GetTG(TxtThoi_Gian_SK.Text, 0)
    End Sub
#End Region
#Region "xác nhận"
    Private Sub V_Xac_nhan(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If M_Mode = "X" And M_Stt_Rec <> "" Then
            ...
            Dim nS As Integer = 0
            If MsgBox("Bạn có muốn chuyển thành khách hàng tiềm năng ?", MsgBoxStyle.YesNo + MsgBoxStyle.Critical, "CYBER9.0") = MsgBoxResult.Yes Then
                ChkXac_nhan.Checked = True
                Ds = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_XacNhanSK1", "1#SK1#" & M_Stt_Rec.Trim)
            End If
...
#Region "Valid - Detail - Add On"
    ...
    End Sub
    Private Sub V_Ma_tp_i(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs)
        CyberSupport.V_Lookup("Ma_TP", "DMTP", "1=1", "1=1", M_Mode, M_Ma_CT, M_LAN, M_Para, osysvar, AppConn, DsLookup, sender)
    End Sub

    Private Sub L_Ma_tp_i(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        DrReturn = Nothing
        ...
        If iRow < 0 Then Exit Sub
        Dv_DetailTmp.Item(iRow).BeginEdit()
        Dv_DetailTmp.Item(iRow).Item("ma_tp") = DrReturn("ma_tp")
        Dv_DetailTmp.Item(iRow).Item("Ten_tp") = DrReturn("Ten_tp")
        Dv_DetailTmp.Item(iRow).EndEdit()
        Dv_DetailTmp.Table.AcceptChanges()
    End Sub
    Private Sub V_Ma_quan_i(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs)
        Dim iRow As Integer = -1
        iRow = DetailGRV.GetFocusedDataSourceRowIndex
        Dim _Ma_TP As String = Dv_DetailTmp.Item(iRow).Item("ma_TP").ToString.Trim
        'MsgBox(Dv_DetailTmp.Item(iRow).Item("ma_tp"))
        CyberSupport.V_Lookup("Ma_quan", "DMquan", "1=1", "Ma_TP = '" + _Ma_TP + "'", M_Mode, M_Ma_CT, M_LAN, M_Para, osysvar, AppConn, DsLookup, sender)
    End Sub

    Private Sub L_Ma_quan_i(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        DrReturn = Nothing
        DrReturn = CyberSupport.L_Lookup(M_Mode, M_Ma_CT, sender, True)
        Dim iRow As Integer = -1
        ...
        Dv_DetailTmp.Item(iRow).Item("ma_quan") = DrReturn("ma_quan")
        Dv_DetailTmp.Item(iRow).Item("Ten_quan") = DrReturn("Ten_quan")
        Dv_DetailTmp.Item(iRow).EndEdit()
        Dv_DetailTmp.Table.AcceptChanges()
    End Sub
    Private Sub V_Ma_xa_i(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs)
        Dim iRow As Integer = -1
        iRow = DetailGRV.GetFocusedDataSourceRowIndex
        Dim _Ma_quan As String = Dv_DetailTmp.Item(iRow).Item("ma_quan").ToString.Trim
        CyberSupport.V_Lookup("Ma_xa", "DMxa", "1=1", "Ma_Quan = '" + _Ma_quan + "'", M_Mode, M_Ma_CT, M_LAN, M_Para, osysvar, AppConn, DsLookup, sender)
    End Sub

    Private Sub L_Ma_xa_i(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        DrReturn = Nothing
        DrReturn = CyberSupport.L_Lookup(M_Mode, M_Ma_CT, sender, True)
        Dim iRow As Integer = -1
        ...
        Dv_DetailTmp.Item(iRow).Item("ma_xa") = DrReturn("ma_xa")
        Dv_DetailTmp.Item(iRow).Item("Ten_xa") = DrReturn("Ten_xa")
    ...
    Private Sub V_Ma_kx(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs)
        CyberSupport.V_Lookup("Ma_kx", "DmKx", "1=1", "Is_KD=1", M_Mode, M_Ma_CT, M_LAN, M_Para, osysvar, AppConn, DsLookup, sender)
    End Sub
    ...
    Private Sub L_Ma_kx(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If iRow < 0 Then Exit Sub
        Dv_DetailTmp.Item(iRow).BeginEdit()
        Dv_DetailTmp.Item(iRow).Item("ma_kx") = DrReturn("ma_kx")
        Dv_DetailTmp.Item(iRow).Item("ten_kx") = DrReturn("ten_kx")

        Dv_DetailTmp.Item(iRow).EndEdit()
        Dv_DetailTmp.Table.AcceptChanges()
    End Sub
    Private Sub V_MA_PTLH(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs)
        CyberSupport.V_Lookup("MA_PTLH", "CRDMPTLH", "1=1", "1=1", M_Mode, M_Ma_CT, M_LAN, M_Para, osysvar, AppConn, DsLookup, sender)
    End Sub

    Private Sub L_MA_PTLH(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        DrReturn = Nothing
        DrReturn = CyberSupport.L_Lookup(M_Mode, M_Ma_CT, sender, True)
        Dim iRow As Integer = -1
        ...
        If iRow < 0 Then Exit Sub
        Dv_DetailTmp.Item(iRow).BeginEdit()
        Dv_DetailTmp.Item(iRow).Item("MA_PTLH") = DrReturn("MA_PTLH")
        Dv_DetailTmp.Item(iRow).Item("Ten_PTLH") = DrReturn("Ten_PTLH")
    ...
    Private Sub V_Ma_mau(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs)
        ...
        CyberSupport.V_Lookup("Ma_mau", "DmMauKx", "1=1", "Ma_kx = '" + _Ma_Kx + "'", M_Mode, M_Ma_CT, M_LAN, M_Para, osysvar, AppConn, DsLookup, sender)
    End Sub
    ...
    Private Sub L_Ma_mau(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If iRow < 0 Then Exit Sub
        Dv_DetailTmp.Item(iRow).BeginEdit()
        Dv_DetailTmp.Item(iRow).Item("ma_mau") = DrReturn("ma_mau")

        Dv_DetailTmp.Item(iRow).EndEdit()
        Dv_DetailTmp.Table.AcceptChanges()
    End Sub
    Private Sub V_Ma_name(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs)

        CyberSupport.V_Lookup("Ma_hs", "Dmhs", "1=1", "Ma_bp = '" + "KD_HONDA" + "'", M_Mode, M_Ma_CT, M_LAN, M_Para, osysvar, AppConn, DsLookup, sender)
    End Sub

    Private Sub L_Ma_name(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        DrReturn = Nothing
        DrReturn = CyberSupport.L_Lookup(M_Mode, M_Ma_CT, sender, True)
        Dim iRow As Integer = -1
    ...
    '    CyberSupport.V_Lookup("Ma_chucdanh", "DMchucdanh", "1=1", "1=1", M_Mode, M_Ma_CT, M_LAN, M_Para, osysvar, AppConn, DsLookup, sender)
    'End Sub
...
#Region "Button"
    ...
    Private Sub V_GetNgamDinh()
        '----------------------------------------------------------------------------
        TxtThoi_Gian_SK.Text = CyberVoucher.V_GetTG("", 0)
    End Sub
    Private Sub V_New()
