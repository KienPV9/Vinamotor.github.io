...
#Region "Columns Edit"
    ...
    Dim EditGia_Vat, EditTien_vat, EditCK_VAT As New Cyber.Fill.CyberColumnGridView
#End Region
...
#Region "Load And Set And Default"
    Private Sub V_Load()
        ...
        TxtT_Tien_VAT.InputMask = IIf(DrDmct.Item("M_IP_MK_Tien_Nt").ToString.Trim <> "", DrDmct.Item("M_IP_MK_Tien_Nt").ToString.Trim, TxtT_Tien_VAT.InputMask)
        TxtT_thue_VAT.InputMask = IIf(DrDmct.Item("M_IP_MK_Tien_Nt").ToString.Trim <> "", DrDmct.Item("M_IP_MK_Tien_Nt").ToString.Trim, TxtT_thue_VAT.InputMask)
        TxtT_CK_VAT.InputMask = IIf(DrDmct.Item("M_IP_MK_Tien_Nt").ToString.Trim <> "", DrDmct.Item("M_IP_MK_Tien_Nt").ToString.Trim, TxtT_CK_VAT.InputMask)
        TxtT_TT_VAT.InputMask = IIf(DrDmct.Item("M_IP_MK_Tien_Nt").ToString.Trim <> "", DrDmct.Item("M_IP_MK_Tien_Nt").ToString.Trim, TxtT_TT_VAT.InputMask)
    ...
    Private Sub V_GetColumn()
        ...
        EditGia_Vat.GetColumn(DetailGRV, "Gia_Vat")
        EditTien_vat.GetColumn(DetailGRV, "Tien_vat")
        EditCK_VAT.GetColumn(DetailGRV, "CK_VAT")
    End Sub
...
#Region "V_AddHandler"
    ...
    Private Sub V_AddHandler_Master()
        ...
        AddHandler TxtT_thue_VAT.Leave, AddressOf L_T_Thue_NT_VAT
        AddHandler TxtT_thue_VAT.Enter, AddressOf E_T_Thue_NT_VAT
        '----------------------------------------------------------------------------------------------------------
    ...
    Private Sub V_AddHandler_Detail()
        ...
        If Not EditGia_Vat.Column Is Nothing Then AddHandler EditGia_Vat.EditColumn.Leave, AddressOf L_Tinh_Toan
        If Not EditTien_vat.Column Is Nothing Then AddHandler EditTien_vat.EditColumn.Leave, AddressOf L_Tinh_Toan
        If Not EditCK_VAT.Column Is Nothing Then AddHandler EditCK_VAT.EditColumn.Leave, AddressOf L_Tinh_Toan
...
#Region "Valid --- Ma_TD3_H"
    ...
    Private Sub CP_GetHDCFromMa_HD()
        ...
        If Dt.Columns.Contains("Sua_CK") Then ChkSua_CK.Checked = Dt.Rows(0).Item("Sua_CK")
        If Dt.Columns.Contains("CK") Then TxtT_CK_Nt.Double = Dt.Rows(0).Item("CK")
        'hieulv
...
#Region "Phieu xuat gia dich danh Sua/HT thue/Tien thue/Chiet khau"
    ...
    End Sub
    Private Sub L_T_Thue_NT_VAT(ByVal sender As System.Object, ByVal e As System.EventArgs)
        M_Mode = M_Mode.Trim
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        UpdateTienVAT()
    End Sub
    Private Sub E_T_Thue_NT_VAT(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If Not (M_Mode.Trim = "M" Or M_Mode.Trim = "S") Then Exit Sub
        If Not Me.ChkSua_Thue.Checked Then SendKeys.Send("{Tab}")
    End Sub
#End Region
...
#Region "Valid - Detail - Core"
    Private Sub V_Tinh_Thue_CK()
        ...
        Dim nT_Thue_VAT As Decimal = TxtT_thue_VAT.Double
        Dim nT_Tien_VAT As Decimal = TxtT_Tien_VAT.Double
        ...
        If Not ChkSua_Thue.Checked Then
            ...
            nT_Thue_VAT = CyberSupport.V_Round(nT_Tien_VAT * nThue_Suat / 100, M_R_TIEN_NT)
        ...
        TxtT_CK.Double = nT_Ck
        '--
        TxtT_thue_VAT.Double = nT_Thue_VAT
    End Sub
    Private Sub UpdateList()
        Dim nT_Tien_Nt2 As Decimal = 0
        Dim nT_Tien2 As Decimal = 0

        Dim nT_Tien_VAT As Decimal = 0
        Dim nT_CK_VAT As Decimal = 0
        Dim nT_T_TT_VAT As Decimal = 0
        ...
        For iRow As Integer = 0 To Dv_DetailTmp.Count - 1
            ...
            If M_Ma_Dvcs = "01" Then
                nT_Tien_VAT = nT_Tien_VAT + Dv_DetailTmp.Item(iRow).Item("Tien_vat")
                nT_CK_VAT = nT_CK_VAT + Dv_DetailTmp.Item(iRow).Item("CK_VAT")
            End If
        ...
        If M_Ma_Dvcs = "01" Then
            TxtT_Tien_VAT.Double = nT_Tien_VAT
            TxtT_CK_VAT.Double = nT_CK_VAT
        End If
        ...
        If M_Ma_Dvcs = "10" Or M_Ma_Dvcs = "01" Then 'SONNT: Merc xử lý giống với DMS
        ...
        '----------------------------------------------------------------------
        nT_T_TT_VAT = nT_Tien_VAT + nT_Thue_VAT - nT_CK_VAT
        TxtT_TT_VAT.Double = nT_T_TT_VAT
        '----------------------------------------------------------------------
        ...
        '----------------------------------------------------------------------
    End Sub
    Private Sub UpdateTienVAT()
        Dim nT_Tien_VAT As Decimal = 0
        Dim nT_Thue_VAT As Decimal = 0
        Dim nT_CK_VAT As Decimal = 0
        Dim nT_T_TT_VAT As Decimal = 0
        Dim nThue_Suat As Decimal = TxtThue_Suat.Double
        '---------------------------------------------
        nT_Tien_VAT = TxtT_Tien_VAT.Double
        nT_Thue_VAT = TxtT_thue_VAT.Double
        nT_CK_VAT = TxtT_CK_VAT.Double
        nT_T_TT_VAT = TxtT_TT_VAT.Double
        '----------------------------------------------------------------------
        nT_T_TT_VAT = nT_Tien_VAT + nT_Thue_VAT - nT_CK_VAT
        '----------------------------------------------------------------------
        TxtT_Tien_VAT.Double = nT_Tien_VAT
    ...
    Private Sub V_Tinh_Toan(ByVal iRow As Integer)
        ...
        End If
        If M_Ma_Dvcs = "01" Then
            If Dv_DetailTmp.Item(iRow).Item("So_Luong") * Dv_DetailTmp.Item(iRow).Item("Gia_Vat") <> 0 Then
                Dv_DetailTmp.Item(iRow).BeginEdit()
                Dv_DetailTmp.Item(iRow).Item("Tien_vat") = CyberSupport.V_Round(Dv_DetailTmp.Item(iRow).Item("So_Luong") * Dv_DetailTmp.Item(iRow).Item("Gia_Vat"), M_R_TIEN_NT)
                Dv_DetailTmp.Item(iRow).EndEdit()
            End If
        End If
        '--------------------------------------------------------------------------------
...
#Region "Sms - Import"
    ...
    Private Sub SetTxt()
        ...
        If M_Ma_Dvcs <> "01" Then
            EditCK_VAT.Visible = False
            EditTien_vat.Visible = False
            EditGia_Vat.Visible = False
            GroupBox3.Visible = False
            Lbtonghang.Visible = False
            Lbtienthue.Visible = False
            Lbck.Visible = False
            Lbtongcong.Visible = False
            TxtT_Tien_VAT.Visible = False
            TxtT_thue_VAT.Visible = False
            TxtT_CK_VAT.Visible = False
            TxtT_TT_VAT.Visible = False
        End If
    End Sub
#End Region
