...
#Region "Columns Edit"
    ...
    Dim EditMa_VT, EditMa_Vitri, EditMa_Kho_I, EditMa_Vv_I, EditMa_phi_i, EditMa_HD_I, EditMa_HD_N_I, EditMa_Sp_I, EditMa_ku_I, EditMa_TTLN_i, EditMa_TTCP_i, EditMa_BP_I, EditMa_HS_I, EditMa_CD_I, EditMa_TD1_I, EditMa_TD2_I, EditMa_TD3_I, EditMa_TD3_N_I, EditMa_TD4_I, EditMa_TD5_I As New Cyber.Fill.CyberColumnGridView 'Cyber.SmLists.EditLookUp
...
#Region "Load And Set And Default"
    ...
    Private Sub V_GetColumn()
        ...
        EditMa_HD_N_I.GetColumn(DetailGRV, "ma_hd_N")
        ...
        EditMa_TD3_N_I.GetColumn(DetailGRV, "ma_Td3_N")
...
#Region "V_AddHandler"
    ...
    Private Sub V_AddHandler_Detail()
        ...
        EditMa_HD_N_I.V_ActiLookUpColumn(AddressOf V_Ma_HD_N_i, AddressOf L_Ma_HD_N_i)
        ...
        EditMa_TD3_N_I.V_ActiLookUpColumn(AddressOf V_Ma_TD3_N_i, AddressOf L_Ma_TD3_N_i)
    ...
    Private Sub VisibleColumnMa_GD()
        Dim ma_gd As String = CbbMa_GD.SelectedValue.ToString.Trim.ToUpper
        If ma_gd = "7" Then
            EditMa_HD_N_I.Visible = True
            EditMa_TD3_N_I.Visible = True
        Else
            EditMa_HD_N_I.Visible = False
            EditMa_TD3_N_I.Visible = False
        End If
    End Sub
...
#Region "Valid - Detail - Add On"
    ...
    Private Sub V_Ma_HD_N_i(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs)
        'CyberSupport.V_Ma_Hd_i(M_Mode, M_Ma_CT, M_LAN, M_Para, osysvar, AppConn, DsLookup, sender)
        CyberSupport.V_Lookup("Ma_HD", "DMHD", "1=1", "1=1", M_Mode, M_Ma_CT, M_LAN, M_Para, osysvar, AppConn, DsLookup, sender)
    End Sub
    Private Sub L_Ma_HD_N_i(ByVal sender As System.Object, ByVal e As System.EventArgs)
        'CyberSupport.L_Ma_Hd_i(M_Mode, M_Ma_CT, sender, DetailGRV)
        Dim iRows As Integer = -1
        Dim drReturn As DataRow
        iRows = DetailGRV.FocusedRowHandle
        ...
        drReturn = CyberSupport.L_Lookup(M_Mode, M_Ma_CT, sender, True)
        If drReturn Is Nothing Then
            Dt_DetailTmp.Rows(iRows)("ma_hd_N") = ""
            Dt_DetailTmp.AcceptChanges()
            Exit Sub
        End If
        Dt_DetailTmp.Rows(iRows)("ma_hd_N") = drReturn("Ma_HD").ToString.Trim
        Dt_DetailTmp.AcceptChanges()
        DetailGRV.UpdateCurrentRow()
        V_GetMa_td3_N(iRows)
    End Sub
    Private Sub V_Ma_Td3_n_i(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs)
        'CyberSupport.V_Ma_Hd_i(M_Mode, M_Ma_CT, M_LAN, M_Para, osysvar, AppConn, DsLookup, sender)
        CyberSupport.V_Lookup("Ma_td3", "dmtd3", "1=1", "1=1", M_Mode, M_Ma_CT, M_LAN, M_Para, osysvar, AppConn, DsLookup, sender)
    End Sub
    Private Sub L_Ma_td3_n_i(ByVal sender As System.Object, ByVal e As System.EventArgs)
        'CyberSupport.L_Ma_Hd_i(M_Mode, M_Ma_CT, sender, DetailGRV)
        Dim iRows As Integer = -1
        Dim drReturn As DataRow
        iRows = DetailGRV.FocusedRowHandle
        ...
        drReturn = CyberSupport.L_Lookup(M_Mode, M_Ma_CT, sender, True)
        If drReturn Is Nothing Then
            Dt_DetailTmp.Rows(iRows)("ma_Td3_N") = ""
            Dt_DetailTmp.AcceptChanges()
            Exit Sub
        End If
        Dt_DetailTmp.Rows(iRows)("ma_Td3_N") = drReturn("Ma_td3").ToString.Trim
        Dt_DetailTmp.AcceptChanges()
        DetailGRV.UpdateCurrentRow()
    End Sub
...
#Region "get ma_td3"
    Private Sub V_GetMa_td3_N(ByVal iRow As Integer)
        ...
        If iRow < 0 Then Exit Sub
        If iRow < 0 Then Exit Sub
        Dim _Ma_HD As String = If(Dt_DetailTmp.Columns.Contains("ma_hd_N"), Dv_DetailTmp.Item(iRow).Item("ma_hd_N").ToString.Trim, "")
        Dim DsMa_td3 As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_GETMa_td3", _Ma_HD & "#" & M_Ma_Dvcs & "#" & M_User_Name)
        If DsMa_td3.Tables(0).Rows.Count = 0 Then Exit Sub
        Dt_DetailTmp.Rows(iRow).BeginEdit()
        Dt_DetailTmp.Rows(iRow).Item("ma_Td3_N") = DsMa_td3.Tables(0).Rows(0).Item("ma_td3")
        Dt_DetailTmp.Rows(iRow).EndEdit()
    End Sub
    Private Sub V_GetMa_td3(ByVal iRow As Integer)
        M_Mode = M_Mode.Trim
        If iRow < 0 Then Exit Sub
        If iRow < 0 Then Exit Sub
