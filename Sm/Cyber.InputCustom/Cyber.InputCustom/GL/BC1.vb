...
#End Region
#Region "Columns Edit"
    Dim EditTk_i, EditTk_No, EditMa_kh_I, EditMa_kh_N As New Cyber.Fill.CyberColumnGridView 'Cyber.SmLists.EditLookUp
    ...
    Dim EditMa_TT, EditCl_Pb, EditTen_Tk_i, EditTen_Tk_No, EditTen_kh_i, EditTen_kh_N, EditSo_hd, EditNgay_HD, EditTien_Hd, EditTien_Hd_NT, EditDa_PB, EditDa_PB_NT, EditCl_Pb_NT, EditTy_Gia_HT As New Cyber.Fill.CyberColumnGridView 'DevExpress.XtraEditors.Repository.RepositoryItemTextEdit
#End Region
...
#Region "V_AddHandler"
    ...
    Private Sub V_AddHandler_Master()
        ...
        AddHandler CmdLoc_TT.Click, AddressOf V_Loc_TT
...
#End Region
#Region "Valid --- Lọc TT"
    Private Sub V_Loc_TT(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If Not (M_Mode.Trim = "M" Or M_Mode.Trim = "S") Then Exit Sub
        Dim frm As New FrmLocTT
        ...
        TxtLenh_RO.Text = ""
        Dt_DetailTmp.Clear()
        CyberSmodb.SQLTbToTb(frm.CV_Return, Dt_DetailTmp)
...
#Region "Valid --- Ma_HD,Ma_TD3"
    ...
    Sub Get_Ma_HD()
        ...
        dsTMp = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_GetPT1FromMa_Hd", TxtMa_HD_H.Text & "#" & TxtMa_Td3_H.Text & "#" & M_Mode & "#" & M_Stt_Rec & "#" & M_Ma_CT & "#" & _Ngay_CT.ToString("yyyyMMdd") & "#" & CbbMa_GD.SelectedValue & "#" & M_Ma_Dvcs & "#" & M_User_Name)
        ...
        If Dt.Columns.Contains("Ngay_Hd") Then TxtNgay_Hd.Value = Dt.Rows(0).Item("Ngay_Hd")
        If Dt.Columns.Contains("T_Tien_HD") Then TxtT_Tien_HD.Double = Dt.Rows(0).Item("T_Tien_HD")
        If Dt.Columns.Contains("T_Tien_DT") Then TxtT_Tien_DT.Double = Dt.Rows(0).Item("T_Tien_DT")
        If Dt.Columns.Contains("T_Tien_CL") Then TxtT_Tien_CL.Double = Dt.Rows(0).Item("T_Tien_CL")
        If Dt.Columns.Contains("Ma_KH") Then TxtMa_Kh.Text = Dt.Rows(0).Item("Ma_KH")
        If Dt.Columns.Contains("Ten_KH") Then TxtTen_kh.Text = Dt.Rows(0).Item("Ten_KH")
        If Dt.Columns.Contains("Dia_Chi") Then TxtDia_Chi.Text = Dt.Rows(0).Item("Dia_Chi")
        If Dt.Columns.Contains("Dien_giai") Then txtDien_giai.Text = Dt.Rows(0).Item("Dien_giai")
        If Dt.Columns.Contains("ong_ba") Then txtOng_ba.Text = Dt.Rows(0).Item("ong_ba")
...
#Region "Valid - Detail - Core"
    ...
    Private Sub V_Ma_TT(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs)
        CyberSupport.V_Ma("Ma_TT", "DmTT", "1=1", "1=1", M_Mode, M_Ma_CT, M_LAN, M_Para, osysvar, AppConn, Me.DsLookup, sender)
    End Sub
    Private Sub L_Ma_TT(ByVal sender As System.Object, ByVal e As System.EventArgs)
        Dim _Return As DataRow = CyberSupport.L_Ma("Ma_TT", "Ma_TT", "Ten_TT", "Ten_TT", M_Mode, M_Ma_CT, sender, DetailGRV)
        If _Return Is Nothing Then Exit Sub
        Dim iRow As Integer = -1
        iRow = DetailGRV.GetFocusedDataSourceRowIndex
        If iRow < 0 Then Exit Sub
        If Dv_DetailTmp.Item(iRow).Item("Ma_TT").ToString.Trim = "" Then
            Dv_DetailTmp.Item(iRow).BeginEdit()
            Dv_DetailTmp.Item(iRow).Item("Ma_TT") = Dv_DetailTmp.Item(iRow).Item("Ma_TT")
            Dv_DetailTmp.Item(iRow).EndEdit()
            Dv_DetailTmp.Table.AcceptChanges()
        End If
    End Sub
    ...
    Private Sub L_Tk_No(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If Dv_DetailTmp.Item(iRow).Item("Tk_No").ToString.Trim = "" Then
            Dv_DetailTmp.Item(iRow).BeginEdit()
            Dv_DetailTmp.Item(iRow).Item("Tk_No") = Dv_DetailTmp.Item(iRow).Item("Tk_No")
            Dv_DetailTmp.Item(iRow).EndEdit()
            Dv_DetailTmp.Table.AcceptChanges()
        End If
    End Sub
    ...
    Private Sub L_Ma_KH_N(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If Dv_DetailTmp.Item(iRow).Item("Ma_Kh_N").ToString.Trim = "" Then
            Dv_DetailTmp.Item(iRow).BeginEdit()
            Dv_DetailTmp.Item(iRow).Item("Ma_Kh_N") = Dv_DetailTmp.Item(iRow).Item("Ma_Kh_N")
            Dv_DetailTmp.Item(iRow).EndEdit()
            Dv_DetailTmp.Table.AcceptChanges()
        End If
    End Sub
