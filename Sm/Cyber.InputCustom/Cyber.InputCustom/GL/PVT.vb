...
#Region "Columns Edit"
    ...
    Dim EditTen_Vt, EditDvt, EditTen_kho_i, EditTen_Tk_VT, EditNgay_Ban, EditTen_Nx_I, EDITNH_VT1 As New Cyber.Fill.CyberColumnGridView
#End Region
...
#End Region
    Private Sub PVT_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        V_Load()
        V_GetColumn()
        V_AddHandler()
        V_Databing()
...
#Region "V_AddHandler"
    ...
    Private Sub VisibleColumnMa_GD()
        ...
        'M_Ma_GD = CbbMa_GD.SelectedValue
        'If M_Ma_GD = "9" Then
        '    EditNgay_Ban.Visible = True
        'Else
        '    EditNgay_Ban.Visible = False
        'End If
...
#Region "Vat tu kho/Vitri - "
    ...
    Private Sub L_Ma_Vt(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        Dv_DetailTmp.Item(iRow).BeginEdit()
        Dv_DetailTmp.Item(iRow).Item("Ma_VT") = DrReturn.Item("Ma_VT")
        Dv_DetailTmp.Item(iRow).Item("Ten_VT") = DrReturn.Item("Ten_VT")
        'Dv_DetailTmp.Item(iRow).Item("Dvt") = DrReturn.Item("Dvt")
        Dv_DetailTmp.Item(iRow).EndEdit()
        Dv_DetailTmp.Table.AcceptChanges()
        'V_SetSD(iRow)
    End Sub
...
#Region "Button"
    ...
    Private Function ChkVsave() As Boolean
        ...
        For iRow = nCount - 1 To 0 Step -1
            If Dt_DetailTmp.Rows(iRow).Item("Ma_VT").ToString.Trim = "" Or Dt_DetailTmp.Rows(iRow).Item("NH_VT1").ToString.Trim = "" Then
...
#Region "Sms - Import"
    ...
    End Sub
#End Region
    'Private Sub V_GetDetail()
    '    If Not M_Mode = "M" Or M_Mode = "S" Then Exit Sub
    '    If Dv_DetailTmp.Count > 1 Then Exit Sub

    '    Dim dt As Date = TxtNgay_Ct.Value
    '    Dim DsTmpDetal As New DataSet
    '    Dim nCountTb As Integer

    '    DsTmpDetal = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_GetDetailBTT", M_Mode.Trim + "#" + M_Stt_Rec.Trim + "#" + M_Ma_GD.Trim + "#" + M_Ma_CT.Trim + "#" + _
    '          dt.ToString("yyyyMMdd") + "#" + TxtMa_HD.Text.Trim + "#" + TxtMa_TD5.Text.Trim + "#" + M_Ma_Dvcs.Trim + "#" + M_User_Name)

    '    nCountTb = DsTmpDetal.Tables.Count
    '    For i = 0 To nCountTb - 1
    '        CyberSmodb.SetNotNullTable(DsTmpDetal.Tables(i))
    '    Next
    '    If Not CyberSupport.V_MsgChk(DsTmpDetal.Tables(0), osysvar, M_LAN) Then Exit Sub
    '    Dt_DetailTmp.Clear()
    '    Dim _Dt_Update As DataTable = DsTmpDetal.Tables(1)

    '    CyberSmodb.SQLTbToTb(_Dt_Update, Dt_DetailTmp)
    '    DsTmpDetal.Dispose()
    'End Sub
    Private Sub V_CreatAutoDMVT()
        M_Ma_GD = CbbMa_GD.SelectedValue
        'If Not M_Mode = "M" And Not M_Mode = "S" Then Exit Sub
        If Not M_Mode = "M" Then Exit Sub
        Dim dt As Date = TxtNgay_Ct.Value
        CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_CreateAutoDMVTForExcel", M_Mode.Trim + "#" + M_Stt_Rec.Trim + "#" + M_Ma_GD.Trim + "#" + M_Ma_CT.Trim + "#" + _
              dt.ToString("yyyyMMdd") + "#" + M_Ma_Dvcs.Trim + "#" + M_User_Name)
    End Sub
End Class
