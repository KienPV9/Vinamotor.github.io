...
Public Class CRQLKHTN
    ...
    Private TbTP, TbQuan, TbXa, tbTNHN, tbLHKH, tbGT, tbKX, tbPTLH, tbDT, tbTTKH, tbKhmua, tbhttt, tbmau, tbmau_nt As DataTable
    ...
    Private Sub FDKHTN_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        'SONNT: set Enabled thôngng tin môi giới theo user
        Dim Dstmp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(Appconn, "CP_CheckEnabledQLKHTN", Mode + "#" + M_Ma_Dvcs + "#" + M_User_Name)
        Dim _Result As String = Dstmp.Tables(0).Rows(0).Item("Resul").ToString.Trim
        If Dstmp.Tables(0).Rows.Count > 0 Then
            If _Result = "0" Then
                Txtnguoi_mg.Enabled = False
                Txtdt_mg.Enabled = False
                Txtdc_mg.Enabled = False
            Else
                Txtnguoi_mg.Enabled = True
                Txtdt_mg.Enabled = True
                Txtdc_mg.Enabled = True
            End If
...
#Region "LoadCombobox"
    ...
    Private Sub V_LoadMa_Mau()
        ...
        Dim _ma_Mau_nt As String = TxtMa_Mau_Nt.Text
        Try
            _ma_Kx = TxtMa_KX.Text
        Catch ex As Exception
        End Try
        V_FillMa_Mau(_ma_Kx, _ma_Mau, "1")
        V_FillMa_Mau_NT(_ma_Kx, _ma_Mau_nt, "1")
    End Sub
#End Region
#Region "Load"
    ...
    Protected Sub V_AddhanderMaster()
        ...
        'AddHandler TxtDia_Chi.Leave, AddressOf V_Dia_Chi_Nha
        ''---TInh thanh
...
#Region "Mau xe"
    Private Sub V_Ma_Kx_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        V_FillMa_Mau_NT(_ma_Kx, _ma_Mau, "0")
    End Sub
    ...
    End Sub
    Private Sub V_FillMa_Mau_NT(ByVal _Ma_Kx As String, ByVal _Ma_Mau_nt As String, ByVal _Load As String)
        'Dim DsTbma_mau As DataSet = CyberSmlib.SQLExcuteStoreProcedure(Appconn, "CP_CRQL_KX_MAU", _Ma_Kx & "#" + _Ma_Mau & "#" & M_Ma_Dvcs & "#" & M_User_Name)

        If _Load = "1" Then
            tbmau_nt = V_GetTbMau_nt(_Ma_Kx, _Ma_Mau_nt)
            CyberFill.V_FillComBoxDefaul(Me.CmbbMa_Mau_nt, tbmau_nt, "Ma_Mau", "Ten_Mau", "Ngam_Dinh")
        Else
            Dim _DT As DataTable = V_GetTbMau_nt(_Ma_Kx, _Ma_Mau_nt)
            tbmau_nt.Clear()
            For i As Integer = 0 To _DT.Rows.Count - 1
                tbmau_nt.ImportRow(_DT.Rows(i))
            Next
            tbmau_nt.AcceptChanges()
            Try
                CmbbMa_Mau_nt.SelectedValue = _Ma_Mau_nt
            Catch ex As Exception
    ...
    Private Function V_GetTbMau(ByVal _ma_kx As String, ByVal _Ma_Mau As String) As DataTable
        ...
        V_GetTbMau = _Dt
    End Function
    Private Function V_GetTbMau_nt(ByVal _ma_kx As String, ByVal _Ma_Mau_nt As String) As DataTable
        Dim DsTbma_mau As DataSet = CyberSmlib.SQLExcuteStoreProcedure(Appconn, "CP_CRQL_KX_MAU_nt", _ma_kx & "#" + _Ma_Mau_nt & "#" & M_Ma_Dvcs & "#" & M_User_Name)
        Dim _Dt As DataTable = DsTbma_mau.Tables(0).Copy
        DsTbma_mau.Dispose()
        V_GetTbMau_nt = _Dt
    End Function
...
#Region "Valid"
    ...
    Private Sub V_Dia_Chi(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If TxtDia_ChiVAT.Text = "" And TxtDia_Chi.Text <> "" Then
            TxtDia_ChiVAT.Text = TxtDia_Chi.Text
        End If
    End Sub
...
#Region "Buuton"
    Protected Overrides Sub V_Save(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        Try
            ...
            TxtMa_Mau_nt.Text = CmbbMa_Mau_nt.SelectedValue.ToString.Trim
        Catch ex As Exception
